...
- La Dashboard evita doppioni: il piano oggi/domani e l'unica sezione previsionale, mentre il meteo attuale resta compatto nello stato operativo.
- Le pagine di configurazione mostrano gli errori di salvataggio direttamente in pagina, senza popup invasivi.
- Ogni ciclo mostra una anteprima decisionale oggi/domani con cosa farebbe la logica ET/meteo.
- Ogni ciclo ha un registro eventi dedicato e richiudibile, oltre alla diagnostica generale.
- I cicli si configurano con step strutturati: zona, durata `hh:mm:ss`, aggiunta e rimozione righe.
- Gli orari di partenza dei cicli si configurano con input orario guidato e pulsanti aggiungi/rimuovi.
...
- La simulazione mostra per ogni zona la formula testuale che spiega come sono stati calcolati i minuti.
...
- Un toggle Avanzate persistente nasconde o mostra pagine e impostazioni tecniche.
...
- Le metriche operative usano icone colorate e tooltip per ridurre il testo ripetitivo.
- Il forecast Home Assistant viene letto anche dal formato `service_response` usato da `weather.get_forecasts`.
- La pagina Meteo include una verifica forecast in tempo reale per hourly, daily e disponibilita di domani.
- La UI mobile usa navigazione compatta in basso e tabelle impilate per schermi stretti.
